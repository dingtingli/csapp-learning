...
[上一篇笔记](./12条件控制.md)中我们介绍了跳转指令，并且详细讲解了如何使用跳转指令实现编程语言中的 `if` 语句。
...
编译后生成的指令所使用的控制流，跟 c 语言代码有些不同，我们同样使用 c 语言来描述一下指令所使用的控制流。
...
通过 GCC 的 `-Og` 选项进行编译，生成第一种指令的翻译形式，称之为jump to middle。

这种形式将 `while` 的执行内容放到中间，首先无条件跳转到结尾，在结尾处再进行条件判断。具体的控制流形式可以用一段 c 语言表示：
...
将这个 `while` 指令的形式与 `do…while` 进行对比，我们发现整体结构很类似，只是在开始是添加了一个无添加跳转，然后跳转处进行条件判断，并再次跳转到中间，这和它的名字 jump to middle 很契合。
...
使用 GCC 的 `-O1` 进行优化编译时，生成了第二种指令翻译形式，称之为 guarded-do。
...
有没有发现，这时候将 `while`中的判断进行了反转，从 `n>1` 变成了 `n<=1`，这样的变化我们在[上一篇笔记](./12条件控制.md) `if` 生成的指令中也曾经看到过。通常这会带来更简洁高效的指令。

另外我们发现最后在判断 `n>1` 时，指令使用了 `n!=1`，因为编辑器知道在做完 `n<=1`的判断后，只要保证 `n!=1` 就能满足 `n>1` 这个条件。而且 `n!=1` 执行更快。
...
其实上面的代码完全可以使用 `while` 语句来替代 `for` 语句。
