...
本文通过书本的作者 Randal E. Bryant 在课堂<sup id="jump01">[1]</sup>中使用的一个示例为基础，来探讨内存的布局是什么样子。
...
*注意：示例中省略了输出功能，完整的示例代码我放在了 Github <sup id="jump02">[2]</sup>上。
...
在 Linux 环境中，我们还可以使用 pmap <sup id="jump02">[2]</sup>（report memory map of a process）命令，来查看虚拟内存具体的使用情况。

上面的示例，在代码中插入 puase() 语句，可将程序暂停，这时候在命令行窗口使用 pmap 命令。

*注意：示例代码在 Linux 环境中执行。可以使用 WSL 来进行环境配置，具体方法已经发布在 Github 上<sup id="jump03">[3]</sup>。
...
<a name="jump01">[1]</a> [课程视频](https://www.bilibili.com/video/BV1iW411d7hd?p=9)

<a name="jump02">[2]</a> [完整代码和pmap命令](../程序角度的内存布局.md)

<a name="jump03">[3]</a> [WSL 环境配置](../准备Linux环境.md)