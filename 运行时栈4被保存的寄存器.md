...
[上一篇文章](./运行时栈3转移控制.md)中，执行完 `call` 指令后，程序计数器（PC：Program Counter）中下一条指令就是被调用函数 `multstore` 了。
...
`multstore` 中第一条有效的指令是 `pushq   %rbx`，其中操作数 %rbx 的作用是**被调用者保存**。意思就是说接下来的指令会用到 %rbx，所以要先将其中的值保存到 Stack（栈）上，用完之后再使用 `popq    %rbx`指令，将其从 Stack（栈）中复制回 %rbx，从而恢复原样。
