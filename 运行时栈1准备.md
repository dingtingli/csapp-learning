...
其实大多数指令都是可操作的内存空间就是 Stack（栈）和 Heap（堆），其中 Stack（栈）尤其重要，会涉及到很多规则。
...
传递给 `multstore` 函数的有 3 个参数，`2`, `3` 和变量 `d` 的地址 `&d`。

### multstore

函数 `multstore` 首先调用函数 `mult2`，并且将 `main` 函数传来的参数 `2` 和 `3` 作为参数传给函数 `mult2`。

函数 `mult2` 的返回值被赋值给变量 `t`。

最终 `t` 会被写入 `main` 函数传来的地址中，也就是变量 `d` 的地址。

### mult2

`mult2` 将传来的两个参数相乘，并返回相应结果。

所有函数最终的结果，就是将 `2` 和 `3` 的乘积赋值给变量 `d`。
...
这段指令中的大部分指令我们已经介绍给过了，具体作用我们可以通过下面的列表来查看,

*`endbr64` 指令没有实际意义，可以忽略。*
...
另外，这段 c 语言生成的指令不止我们现在看到的这些，完整的指令可以参考资料部分，我们节选了需要的内容，其他内容现阶段可以忽视。
