...
图中除了上面介绍的几个布局之外，在 Heap 上方有一段内存地址保存着 libc ，这部分保存着共享库的内容，比如 C 语言中的 printf 这类共享库中的内容。
...
从上面的介绍中，我们理解了程序对内存的布局。接下来我们可以进一步探索程序在运行时内存的变化，其中最主要是 Stack 区域发生的变化。

结合[上一篇文章](/程序角度的内存.md)的内容，我们可以看到程序视角，内存的详细情况：

<figure>
    <img src="./doc/illustrations/memory/Memory07.png" width="700" alt="memory layout" align="center">
    <figcaption><em>内存详细布局</em></figcaption>
   <br><br>
</figure>

*注：有些教程中使用的环境，程序从 0x400000 开始。
